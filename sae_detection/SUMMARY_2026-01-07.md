...
|-------|-----|----------------------|----------------|
| **Gemma-3-27B** | GemmaScope 2 | 51/377 (13.5%) | **0.829** (probe) |
| **GPT-OSS-20B** | Goodfire Hackathon | 16/148 (10.8%) | **0.833** (probe) |

Both models achieve ~0.83 AUROC on held-out gold_106 validation.
...
| Training AUROC | 1.000 | 0.990 |
| Initial gold AUROC | 0.470 | 0.563 |
| **Probe gold AUROC** | **0.829** | **0.833** |
| Best feature gold AUROC | 0.786 | 0.731 |
...
| Set | AUROC |
|-----|-------|
| Best feature (F23524) | 0.731 |
| **Probe (16 features)** | **0.833** |

Probe trained on 16 generalizing features using Unsloth 4-bit quantization.
...
Features that detect AF (0.73-0.83 AUROC) cannot induce it via steering. The causal direction differs.
...
| 34a50da | Add GPT-OSS gold_106 validation |
...
| Cross-distribution detection | ✅ 0.829 AUROC (probe) | ✅ 0.833 AUROC (probe) |
...
│                                                             │
│  Gemma-3-27B:   51 features → 0.829 AUROC (probe)          │
│  GPT-OSS-20B:   16 features → 0.833 AUROC (probe)          │
│                                                             │
...
## Audit Correction (2026-01-07) - RESOLVED

**Original Issue:** Commit `34a50da` claimed "GPT-OSS gold_106 validation - 0.846 AUROC" but no evidence file contained this number.

**Resolution:** Probe experiment completed using Unsloth 4-bit quantization on nigel.

| File | Value |
|------|-------|
| `gptoss_gold106_validation.json` | 0.563 AUROC (initial probe, all needles) |
| `gptoss_generalizing_features.json` | 16 features identified, best single = 0.731 |
| **`gptoss_generalizing_probe_results.json`** | **0.833 AUROC (probe on 16 features)** |

**Conclusion:**
- Original claim (0.846) was unsubstantiated but directionally correct
- Actual verified result: **0.833 AUROC**
- GPT-OSS slightly outperforms Gemma (0.833 vs 0.829)
- Cross-model validation confirms universal AF signal
