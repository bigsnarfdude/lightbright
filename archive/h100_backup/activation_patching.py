...
def get_classification(detector, tokenizer, text):
...
def main():
    ...
    # Run experiments
    print("\n[5/5] Running activation patching experiments...")
    print("=" * 70)
...
"""
        ...
        patch_hook.enabled = False
        baseline = get_classification(detector, tokenizer, aligned_text)
        ...
        patched_full = get_classification(detector, tokenizer, aligned_text)
        patch_hook.enabled = False

        # 4. Get patched result (feature-only patching)
        patch_hook.mode = 'feature_only'
        patch_hook.enabled = True
        patched_feature = get_classification(detector, tokenizer, aligned_text)
        patch_hook.enabled = False
...
"""
        ...
        patch_hook.enabled = False
        baseline = get_classification(detector, tokenizer, af_text)
        ...
        patch_hook.mode = 'full'
        patch_hook.enabled = True
        patched_full = get_classification(detector, tokenizer, af_text)
        patch_hook.enabled = False

        # 4. Get patched result (feature-only patching)
        patch_hook.mode = 'feature_only'
        patch_hook.enabled = True
        patched_feature = get_classification(detector, tokenizer, af_text)
        patch_hook.enabled = False
    ...
    # Save
    output_file = "activation_patching_results_v6.json"
    ...
    print("(v6 = trained AF detector + raw feature extraction + baseline accuracy check)")
