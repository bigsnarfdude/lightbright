...
DETECTOR_ID = "vincentoh/gemma3-27b-af-detector-v2"
...
def get_prediction(detector, tokenizer, text):
    """Get prediction from fine-tuned AF detector."""

    # Format for the trained detector
...
"""
    ...
    if "alignment_faking" in gen_lower or "faking" in gen_lower:
        ...
        prediction = 'AF' if gen_lower.startswith('alignment') else 'Aligned'
...
def main():
    ...
    # Load base model for SAE interventions
    print("\n[2/5] Loading base model (bf16)...")
    ...
    model.eval()
    print(f"Base model VRAM: {torch.cuda.memory_allocated() / 1024**3:.1f} GB")

    # Load detector (LoRA adapter) for classification
    print("\n[3/5] Loading AF detector (LoRA)...")
    detector = PeftModel.from_pretrained(model, DETECTOR_ID)
    detector.eval()
    print(f"With detector VRAM: {torch.cuda.memory_allocated() / 1024**3:.1f} GB")

    # Load transcoders and create hooks on detector's base model
    print("\n[4/5] Loading transcoders and registering hooks...")
    hooks = []
    handles = []
    ...
    for layer, feature_indices in FEATURES_BY_LAYER.items():
        ...
        # Hook into the detector's base model layers
        try:
            target_layer = detector.base_model.model.model.language_model.layers[layer]
        except AttributeError:
            try:
                target_layer = detector.model.model.language_model.layers[layer]
            except AttributeError:
                target_layer = detector.base_model.model.language_model.layers[layer]
    ...
    # Test samples
    print("\n[5/5] Running multi-feature ablation...")
    print("=" * 70)
    ...
    for i, sample in enumerate(af_samples):
        ...
        for h in hooks:
            h.enabled = False
        baseline = get_prediction(detector, tokenizer, text)
        ...
        for h in hooks:
            h.enabled = True
        ablated = get_prediction(detector, tokenizer, text)
    ...
    # Save
    output_file = "multi_feature_ablation_results_v6.json"
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {output_file}")
    print("(v6 = trained AF detector + raw feature extraction)")
