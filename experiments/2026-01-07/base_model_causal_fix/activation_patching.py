...
def main():
    print("=" * 70)
    print("ACTIVATION PATCHING: Base Model (Corrected)")
    print("=" * 70)
    print(f"\nModel: {MODEL_ID}")
...
"""
        ...
        patch_hook.enabled = False
        baseline = classify_text_simple(model, tokenizer, aligned_text)
        ...
        patched_full = classify_text_simple(model, tokenizer, aligned_text)
        patch_hook.enabled = False

        # 4. Get patched result (feature-only patching)
        patch_hook.mode = 'feature_only'
        patch_hook.enabled = True
        patched_feature = classify_text_simple(model, tokenizer, aligned_text)
        patch_hook.enabled = False
...
"""
        ...
        patch_hook.enabled = False
        baseline = classify_text_simple(model, tokenizer, af_text)
        ...
        patch_hook.mode = 'full'
        patch_hook.enabled = True
        patched_full = classify_text_simple(model, tokenizer, af_text)
        patch_hook.enabled = False

        # 4. Get patched result (feature-only patching)
        patch_hook.mode = 'feature_only'
        patch_hook.enabled = True
        patched_feature = classify_text_simple(model, tokenizer, af_text)
        patch_hook.enabled = False
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SUMMARY: Activation Patching Results (BASE MODEL)")
    print("=" * 70)
    ...
    # Save
    output_file = "activation_patching_base_model.json"
